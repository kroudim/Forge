...
export const getIssueContent = async (issueKey) => {
    console.log('[issue] Fetching issue content for:', issueKey);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error('[issue] API error:', message);
        ...
        console.log('[issue] Returning issue content:', result);
        return result;
    } catch (err) {
        console.error('[issue] Exception while fetching issue content:', err);
        throw err;
    }
};
