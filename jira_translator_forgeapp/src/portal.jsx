...
export const RequestTranslatorPanel = ({
    ...
    // Log context and key
    useEffect(() => {
        console.log('[RequestTranslatorPanel] productContext:', productContext);
        console.log('[RequestTranslatorPanel] issueKey:', issueKey);
    ...
    useEffect(() => {
        const loadIssue = async () => {
            ...
            if (!issueKey) {
                console.error('[RequestTranslatorPanel] No issueKey found:', missingIssueMessage);
            ...
            try {
                console.log(`[RequestTranslatorPanel] Loading issue content for issueKey: ${issueKey}`);
                const content = await getIssueContent(issueKey);
                console.log('[RequestTranslatorPanel] Issue content loaded:', content);
                setIssueContent(content);
                setDisplayContent(content);
            } catch (err) {
                console.error('[RequestTranslatorPanel] Error loading issue content:', err);
                setError(err);
            } finally {
                setLoadingIssue(false);
            }
    ...
    useEffect(() => {
        const runTranslation = async () => {
            if (!issueContent) {
                console.log('[RequestTranslatorPanel] No issue content to translate.');
                return;
            }

            if (language === 'original') {
                console.log('[RequestTranslatorPanel] Language is original, not translating.');
            ...
            try {
                console.log(`[RequestTranslatorPanel] Translating issue segments to ${language}`);
                const translatedSegments = await translateSegments(issueContent.segments, language);
                console.log('[RequestTranslatorPanel] Translated segments:', translatedSegments);
                setDisplayContent(buildTranslatedIssue(issueContent, translatedSegments));
            } catch (err) {
                console.error('[RequestTranslatorPanel] Error translating segments:', err);
                setError(err);
            } finally {
    ...
    if (error) {
        console.error('[RequestTranslatorPanel] Rendering error section:', error);
        return (
    ...
    if (loadingIssue || !displayContent) {
        console.log('[RequestTranslatorPanel] Showing loading spinner...');
        ...
        <Stack space="space.200">
            ...
            <Select
                ...
                onChange={(option) => {
                    ...
                    console.log('[RequestTranslatorPanel] Language changed:', newValue);
